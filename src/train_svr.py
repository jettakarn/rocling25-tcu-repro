...
def build_split(cfg, strategy: str):
    """Shared train/eval split builder (Table 1 strategies)."""
    seed = int(cfg["seed"])
...
def main() -> None:
    ...
    if args.strategy == "train_full_dev":
        print("warning: train_full_dev evaluates on the same labeled_dev set (optimistic).")
    ...
    out = out_dir / f"{enc}_{args.strategy}.json"
    payload = {
